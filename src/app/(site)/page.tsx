...
export default async function AccueilPage() {
  const [featured, tiers, settings] = await Promise.all([
    prisma.menuItem.findMany({ where: { featured: true, active: true }, take: 4 }),
    ...
    <div>
      ...
      <section className="relative px-4.5 pt-[clamp(38px,7vw,74px)] pb-[clamp(54px,8vw,96px)] overflow-hidden bg-[radial-gradient(1100px_640px_at_78%_8%,#FFE2CE_0%,rgba(255,226,206,0)_62%),#FFF4EC]">
        ...
        <div className="max-w-310 mx-auto grid grid-cols-[repeat(auto-fit,minmax(min(100%,330px),1fr))] gap-[clamp(30px,5vw,54px)] items-center relative">
          <div>
            <div className="inline-flex items-center gap-2.25 pl-2.25 pr-3.75 py-1.75 rounded-full bg-white border border-[#F2D3C0] text-[12.5px] font-bold text-[#8A4230]">
              <span className="animate-pulse-ring w-2.25 h-2.25 rounded-full bg-[#21A85B]" />
              Ouvert · Abomey-Calavi, Womey Adjikpegon
            </div>
            ...
            <p className="max-w-[480px] text-[18.5px] leading-[1.55] text-[#6A392C] mt-6 text-pretty">
              Attiéké poisson, riz aileron, alloco brûlant. Commandez en ligne, payez par MoMo, on
              livre à Calavi — et chaque commande vous rapporte des points.
            </p>
            ...
            <div className="flex flex-wrap gap-x-7.5 gap-y-5.5 mt-10">
              <div>
                <div className="font-grifter text-[32px] text-deep">500 F</div>
                <div className="text-[12.5px] text-[#8A6154] font-semibold">le plat le plus doux</div>
              </div>
              <div className="w-px bg-[#EBD0BF]" />
              <div>
                <div className="font-grifter text-[32px] text-deep">20 min</div>
                <div className="text-[12.5px] text-[#8A6154] font-semibold">livraison Womey</div>
              </div>
              <div className="w-px bg-[#EBD0BF]" />
              <div>
                <div className="font-grifter text-[32px] text-deep">312</div>
                <div className="text-[12.5px] text-[#8A6154] font-semibold">membres fidélité</div>
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="animate-float-2 absolute bottom-[6%] -left-[6%] bg-white rounded-[20px] px-4.5 py-3.5 shadow-[0_22px_48px_rgba(122,45,25,.2)]">
              ...
              <div className="font-grifter text-[28px] text-orange">+50 pts</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <Reveal className="rounded-[28px] overflow-hidden bg-ink text-cream grid grid-cols-[repeat(auto-fit,minmax(min(100%,280px),1fr))] items-stretch">
          <div className="p-[clamp(24px,3.6vw,40px)] grid content-center gap-3.5">
            ...
            <p className="text-[15.5px] text-[#D9B7A7] leading-[1.55] max-w-[420px]">
              Plat d&apos;attiéké, chill, shooting, jeux et réseautage à Moriland CITY. Pass 3 500 F.
            </p>
            <div>
              <Link
                href="/evenements"
      ...
      <section className="max-w-310 mx-auto px-4.5 pt-[clamp(56px,9vw,92px)] pb-5">
        <Reveal className="flex items-end justify-between gap-5.5 flex-wrap">
          ...
          <Link href="/menu" className="border-0 bg-transparent text-deep text-[15px] font-extrabold hover:text-orange">
            Voir tout le menu →
          </Link>
        </Reveal>
          ...
          {featured.map((item) => (
            ...
            >
              ...
              <div className="p-4.5 pb-5">
                ...
                <div className="text-[12.5px] text-[#8A6154] mt-1.25">{item.note || item.category}</div>
                <div className="flex items-center justify-between mt-4 gap-2.5">
                  <span className="font-grifter text-2xl text-deep">{fmt(item.price)}</span>
                  <AddToCartButton menuItemId={item.id} name={item.name} />
                </div>
              </div>
            </Reveal>
          ))}
          ...
          <div className="relative grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-[clamp(28px,4vw,46px)] items-center">
            <div>
              ...
              <p className="text-[17px] leading-[1.6] text-[#E4C6B7] max-w-[420px] mt-4.5">
                1 point par {fmt(settings.ptsPerUnit)} dépensés. À {tiers.map((t) => t.threshold).join(", ")}{" "}
                points, un palier se débloque : un lot garanti, plus un tour de la roue de la
                chance.
              </p>
              <Link
                href="/fidelite"
            ...
            <div className="grid gap-3">
              {tiers.map((t) => (
                ...
                >
                  <div className="font-grifter text-[22px] text-orange min-w-18.5">{t.threshold} pts</div>
      ...
      <section className="max-w-310 mx-auto px-4.5 py-5 pb-[clamp(56px,9vw,92px)]">
        <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-5.5">
          {[
            ...
            <Reveal key={s.n}>
              <div className="font-grifter text-[46px] text-orange">{s.n}</div>
              <div className="text-[19px] font-extrabold mt-1.5">{s.title}</div>
              <p className="text-[15px] text-[#6A392C] leading-[1.6] mt-2">{s.body}</p>
            </Reveal>
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,200px),1fr))] gap-2.5 max-w-310 mx-auto mb-[clamp(56px,9vw,86px)] px-4.5">
        {[
          { src: "/photos/commande-emballee.jpg", alt: "Commandes emballées Chez Charly" },
          { src: "/photos/charly.jpg", alt: "Livraison sur un événement" },
          { src: "/photos/bouffe.jpg", alt: "Pâtes Chez Charly à emporter" },
        ].map((p) => (
          <Reveal key={p.src} className="aspect-[4/3] rounded-[20px] overflow-hidden relative">
      ...
      <section className="bg-cream-2 px-4.5 py-[clamp(52px,9vw,86px)]">
        <div className="max-w-310 mx-auto grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-[clamp(30px,5vw,52px)] items-center">
          <Reveal variant="in" className="rounded-[28px] aspect-[4/5] overflow-hidden bg-[#EACFBD] relative">
            <Image src="/photos/stand.jpg" alt="La salle Chez Charly, mur peint Attiéké" fill className="object-cover" />
          </Reveal>
          <Reveal>
            <div className="text-xs font-extrabold tracking-[.16em] text-deep">LA MAISON</div>
            ...
            <p className="text-[17.5px] leading-[1.65] text-[#6A392C] max-w-[520px] mt-5 text-pretty">
              Chez Charly, on cuisine comme à la maison : poisson braisé du jour, alloco doré à la
              minute, attiéké frais. Notre promesse tient en trois mots, et ils sont sur
              l&apos;enseigne — toujours bien chargé.
            </p>
            <div className="grid grid-cols-2 gap-4 mt-7.5 max-w-[520px]">
              <div className="bg-white rounded-[18px] p-4.5">
                <div className="text-[13px] font-extrabold text-deep">Ouvert tous les jours</div>
                <div className="text-sm text-[#6A392C] mt-1">10 h — 23 h</div>
              </div>
              <div className="bg-white rounded-[18px] p-4.5">
                <div className="text-[13px] font-extrabold text-deep">Sur place & à emporter</div>
                <div className="text-sm text-[#6A392C] mt-1">32 couverts</div>
              </div>
            </div>
          </Reveal>
        </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-[clamp(30px,4vw,44px)] items-center">
          <Reveal>
            ...
            <div className="grid gap-4.5 mt-7 max-w-[460px]">
              <div>
                <div className="text-xs font-extrabold tracking-[.12em] text-label-2">TÉLÉPHONE</div>
                <a href="tel:+2290161555722" className="font-grifter text-[30px] text-deep">
                  +229 01 61 55 57 22
                </a>
              </div>
              <div>
                <div className="text-xs font-extrabold tracking-[.12em] text-label-2">ADRESSE</div>
              ...
              <div>
                <div className="text-xs font-extrabold tracking-[.12em] text-label-2">LIVRAISON</div>
                <div className="text-[17px] text-[#4A2318]">Calavi, Womey, Godomey et Cotonou</div>
              </div>
            </div>
          </Reveal>
          <Reveal
